...
class UsageRepository(BaseRepository):
    ...
    def get_file_upload_usage(self, user_uuid: str) -> Optional[Dict[str, Any]]:
        """
        Get file upload used and file upload limit from usage table via user UUID

        Args:
            user_uuid: UUID of the user

        Returns:
            Dict with file_size_used and file_upload_size_limit, or None if not found
        """
        try:
            usage = self.session.query(Usage).join(User).filter(
                User.id == user_uuid
            ...
            return {
                ...
                'file_upload_size_limit': usage.file_upload_size_limit
            }

        except Exception as e:
            logger.error(f"Error getting file upload usage for user {user_uuid}: {e}")
            raise

    def initialize_file_usage_if_not_exists(self, user_uuid: str, current_file_size: int) -> Dict[str, Any]:
        """
        Initialize usage table with current file size if not existed earlier

        Args:
            user_uuid: UUID of the user
            current_file_size: File size in bytes to initialize with
        ...
        try:
            ...
            new_usage = Usage(
                id=usage_id,
                user_id=user_uuid,
                file_size_used=current_file_size,
                transcription_reset_date=reset_date
            )
            ...
            return {
                'file_size_used': new_usage.file_size_used,
                'file_upload_size_limit': new_usage.file_upload_size_limit,
                'created': True
            }

        except Exception as e:
            self.session.rollback()
            logger.error(f"Error initializing file usage for user {user_uuid}: {e}")
            raise

    def update_file_upload_usage(self, user_uuid: str, additional_file_size: int) -> Dict[str, Any]:
        """
        Update file upload used by adding additional file size

        Args:
            user_uuid: UUID of the user
            additional_file_size: File size in bytes to add to current usage
        ...
        try:
            ...
            # Update file upload used
            usage.file_size_used += additional_file_size
            usage.updated_at = datetime.now()
            ...
            return {
                'file_size_used': usage.file_size_used,
                'file_upload_size_limit': usage.file_upload_size_limit,
                'file_size_bytes_added': additional_file_size,
                'updated': True
            }

        except Exception as e:
            self.session.rollback()
            logger.error(f"Error updating file upload usage for user {user_uuid}: {e}")
            raise