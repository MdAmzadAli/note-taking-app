...
export default function NoteEditorScreen({ note, onSave, onBack }: NoteEditorScreenProps) {
  const [title, setTitle] = useState(note?.title || '');
  const [content, setContent] = useState(note?.content || '');
  const [selectedTheme, setSelectedTheme] = useState(note?.theme || 'default');
  const [showColorPicker, setShowColorPicker] = useState(false);
  const [showMediaModal, setShowMediaModal] = useState(false);
  const [images, setImages] = useState<ImageAttachment[]>(note?.images || []);
  ...
  const handleTakePhoto = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets[0]) {
        ...
        console.log('Photo added successfully:', newImage.id);
      }
    } catch (error) {
  ...
  const handleAddImage = async () => {
    try {
      ...
      console.log('Media library permission granted, launching image picker...');
      ...
      if (!result.canceled && result.assets && result.assets[0]) {
        ...
        setImages(prev => [...prev, newImage]);
        console.log('Image added successfully:', newImage.id);
      }
    } catch (error) {
  ...
  // Handle drawing (placeholder)
  const handleDrawing = () => {
    console.log('Drawing feature - coming soon');
    Alert.alert('Coming Soon', 'Drawing feature will be available in a future update.');
  };

  // Handle recording (placeholder)
  const handleRecording = () => {
    console.log('Recording feature - coming soon');
    Alert.alert('Coming Soon', 'Recording feature will be available in a future update.');
  };

  // Handle tick boxes (placeholder)
  const handleTickBoxes = () => {
    console.log('Tick boxes feature - coming soon');
    Alert.alert('Coming Soon', 'Tick boxes feature will be available in a future update.');
  };
          ...
          {selectedImageIndex !== null && images[selectedImageIndex] && (
            <View style={styles.fullImageContainer}>
              ...
              <TouchableOpacity
                ...
                onPress={() => handleDeleteImage(images[selectedImageIndex].id)}
              >
